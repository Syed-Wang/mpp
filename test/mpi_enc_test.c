...
#define MODULE_TAG "mpi_enc_test" // 模块标签，用于打印日志
 ...
 */
MPP_RET
test_mpp_enc_cfg_setup(MpiEncMultiCtxInfo* info)
{
    ...
    while (!p->pkt_eos) {
        ...
        if (p->osd_enable || p->user_data_enable || p->roi_enable) {
            if (p->user_data_enable) {
                ...
                static RK_U8 uuid_debug_info[16] = { 0x57, 0x68, 0x97, 0x80, 0xe7, 0x0c, 0x4b, 0x65,
                ...
                if (p->fp_output) {
                    ...
                    if (len) {
                        ...
                        if (len < RET_LEN) {
                            ...
                            pkt_long = len + sizeof(RTP_FIXED_HEADER) + sizeof(NALU_HEADER);
...
// 使用方法：./mpi_enc_test -w 1280 -h 720 -f 30 -t 7 -i ./1080.mp4 -o ./out.h264
// -w：视频宽度，1080 pixels
// -h：视频高度，720 p
// -f：视频帧率，30fps
// -t：视频类型，7 为 H264
